  ...
  const goto_tracet &goto_trace)
{
      ...
      it++)
  {
    ...
    case goto_trace_stept::ASSIGNMENT:
         ...
         (it->pc->is_other() && it->lhs_object.is_not_nil()))
      {
        ...
        else
        {
          ...
          std::cout << "assignment 2: " << from_expr(ns, identifier, it->lhs_object) << "-->" << from_expr(ns, identifier, it->lhs_object_value) << std::endl;
        }
    ...
    case goto_trace_stept::DEAD:
      show_state_header(out, *it, it->pc->source_location, it->step_nr);
      out << "FUNCTION_CALL FUNCTION_RETURN SPAWN MEMORY_BARRIER ATOMIC_BEGIN ATOMIC_END DEAD \n";
      break;
