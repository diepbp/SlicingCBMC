...
public:
  ...
  */
  ...
  virtual std::vector<std::pair<int, Slice_Type>> slice_variable_instruction(
  		const namespacet &ns,
  		const irep_idt &identifier,
  		  std::vector<variable_struct> selected_variables,
  		  std::vector<variable_struct> variables) const = 0;
...
Function: goto_program_templatet<codeT, guardT>::slice
...
\*******************************************************************/
//template <class codeT, class guardT>
//std::vector<std::pair<int, Slice_Type>> goto_program_templatet<codeT, guardT>::slice_variable(
//  const namespacet &ns,
//  std::vector<variable_struct> selected_variables,
//  std::vector<variable_struct> variables) const
//{
//	std::cout<<"Size vars: " << variables.size() << std::endl;
...
//			std::cout << it->source_location << " -- tang tang tang -- " << line << std::endl;
//			Slice_Type type = slice_variable_instruction(ns, selected_variables, variables,
//			    it);
...
Function: goto_program_templatet<codeT, guardT>::get_total_line
...
\*******************************************************************/
template <class codeT, class guardT>
int goto_program_templatet<codeT, guardT>::get_total_line(
		const namespacet &ns,
		const irep_idt &identifier
		) const
{
  	...
  	if(!it->source_location.is_nil())
		{
			result = std::max(result,
			    std::stoi(as_string(it->source_location.get_line()), nullptr));
		}
  return result;
}
...
Function: goto_program_templatet<codeT, guardT>::get_CFG
...
\*******************************************************************/
template <class codeT, class guardT>
std::vector<std::vector<int>> goto_program_templatet<codeT, guardT>::get_CFG(
		const namespacet &ns,
		const irep_idt &identifier,
		int total_line) const
{
	std::vector<std::vector<int>> CFG(total_line);
	std::vector<std::pair<int, int>> goto_list;
	std::vector<std::pair<int, int>> target_list;
	int prev_line = 0;
      ...
      it++)
	{
		if(!it->source_location.is_nil())
		{
			...
			int line = std::stoi(as_string(it->source_location.get_line()), nullptr);
			if (prev_line != line && prev_line >= 0)
				CFG[prev_line].push_back(line);

			signed targetNumber = -1;
			if(it->is_target())
			{
				targetNumber = it->target_number;
				target_list.push_back(std::make_pair(line, targetNumber));
			}
			...
			if (it->type == GOTO)
			{
				for(typename instructiont::targetst::const_iterator
				        ...
				        gt_it++)
				{
					signed targetNumber = (*gt_it)->target_number;
					goto_list.push_back(std::make_pair(line,  targetNumber));
				}
			}
			...
			if (it->type == GOTO)
			{
				...
				{
					prev_line = -1;
					std::cout << "GOTO is nill" << std::endl;
					continue;
				}
	...
	for (int i = 0; i < goto_list.size(); ++i)
	{
		...
		{
			if  (goto_list[i].second == target_list[j].second)
			{
				if (goto_list[i].first != target_list[j].first)
					CFG[goto_list[i].first].push_back(target_list[j].first);
				break;
			}
		}
	}
