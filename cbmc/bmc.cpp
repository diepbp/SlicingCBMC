...
//			std::cout << "Next line: " << line << " " << line2 << std::endl;
			...
			for (start_b = 0; start_b < line_string.size(); ++start_b)
				if (line_string[start_b] == '(')
					break;

			for (finish_b = line_string.size() - 1; finish_b > 0; --finish_b)
				if (line_string[finish_b] == ')')
					break;

			expr_condition = line_string.substr(start_b + 1, finish_b - 1 - start_b);
...
//			std::cout << "start vs finish: " << start_bracket - 1 << " " << finish_bracket - 1 << std::endl;
			if (start_bracket == -1
			    || (start_bracket < line2 && finish_bracket > line2)
			    || (start_bracket > line2 && line2 > 0))
				expr_final = "(" + expr_condition + ") == " + "1";
