...
//    						    << from_expr(it->original_lhs_object) << std::endl
//    						    << from_expr(it->ssa_full_lhs) << std::endl
//    						    << from_expr(it->original_full_lhs) << std::endl;
...
//      std::cout << "convert_decls " << from_expr(it->cond_expr) << std::endl;
    }
  }
}
  ...
  prop_convt &prop_conv)
{
  for(SSA_stepst::iterator it=SSA_steps.begin();
      it!=SSA_steps.end(); it++)
  {
//  	std::cout << "Convert guard " << from_expr(it->guard) << " " << from_expr(it->cond_expr)<< std::endl;
    if(it->ignore)
  ...
  prop_convt &prop_conv)
{
      ...
      it!=SSA_steps.end(); it++)
  {
    if(it->is_assume())
    {
//    	std::cout << "convert_assumptions " << from_expr(it->cond_expr) << std::endl;
      if(it->ignore)
  ...
  decision_proceduret &decision_procedure) const
{
      ...
      it++)
  {
    if(it->is_constraint())
    {
      if(it->ignore)
        continue;
//      std::cout << "convert_constraints " << from_expr(it->cond_expr) << std::endl;
//      decision_procedure.set_to_true(it->cond_expr);
      decision_procedure.set_to_true(it->cond_expr);
    }
  }
...
\*******************************************************************/
void symex_target_equationt::update_assertions(
		prop_convt &prop_conv,
		...
		const exprt old_expr)
{
	...
	languages.from_expr(new_expr, new_string);
	languages.from_expr(old_expr, old_string);

	std::cout << "Update assertions:" << new_string << " " << old_string << std::endl;
  ...
  std::cout << "Number of assertions: " << number_of_assertions << std::endl;
  ...
  // We do (NOT a1) OR (NOT a2) ...
	...
	literalt old_literal;
	for (SSA_stepst::iterator it = SSA_steps.begin(); it != SSA_steps.end(); it++)
	{
		if (it->is_assert())
		{
			std::string current_expr;
			languages.from_expr(it->cond_expr, current_expr);
			std::cout << "we are trying to update: " << current_expr << std::endl;
			if (current_expr.compare(old_string) == 0)
			{
//				it->cond_expr = new_expr;
//				old_literal = it->cond_literal;

//				prop_conv.set_frozen(!old_literal);

//				prop_conv.set_to_false(literal_exprt(!old_literal));

//				std::cout << "we are updating: " << current_expr << std::endl;

				implies_exprt implication(assumption, old_expr);

				// do the conversion
//				it->cond_literal = prop_conv.convert((implication));

//				disjuncts.push_back(literal_exprt(!it->cond_literal)); // original
			}
			else
			{
  ...
  prop_convt &prop_conv)
{
  ...
  // We do (NOT a1) OR (NOT a2) ...
    ...
    std::cout << "disjuncts.size: " << disjuncts.size() << std::endl;
    // the below is 'true' if there are no assertions
    prop_conv.set_to_true(disjunction(disjuncts));
}
